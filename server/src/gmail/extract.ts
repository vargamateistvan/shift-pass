...
const RESET_HINTS =
  /(reset|password|recover|verify|confirm|set[-\s]?up|account)/i;
const TOKEN_HINTS =
  /(reset|token|verify|confirm|recover|password|set|change|action)/i;
...
export function looksLikeReset(
  ...
  return (
    (fromMatch && (subjectMatch || bodyLink)) || (subjectMatch && bodyLink)
  );
}
