...
export async function searchMessages(token: string, q: string): Promise<GmailMessage[]> {
  const res = await gmailFetch(token, `/messages?maxResults=10&q=${encodeURIComponent(q)}`);
