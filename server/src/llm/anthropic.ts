...
export function mapToolUse(name: string, input: Record<string, unknown>): MappedAction {
  if (name === 'navigate') {
    return { kind: 'action', action: { action: 'navigate', url: String(input.url ?? '') } };
  }
  if (name === 'finish') {
  ...
  const coord = Array.isArray(input.coordinate) ? (input.coordinate as number[]) : [0, 0];
  switch (ca) {
    case 'left_click':
    case 'double_click':
      return { kind: 'action', action: { action: 'left_click', x: coord[0], y: coord[1] } };
    case 'type':
      return { kind: 'action', action: { action: 'type', text: String(input.text ?? '') } };
    case 'key':
      return { kind: 'action', action: { action: 'key', key: String(input.text ?? '') } };
    case 'scroll': {
