...
export async function runAgentGoal(
  ...
  const messages: Msg[] = [
    {
      role: 'user',
      content: [{ type: 'text', text: `GOAL: ${opts.goal}` }, obs.image, { type: 'text', text: obs.text }],
    },
  ];

  for (let step = 1; step <= config.maxAgentSteps; step += 1) {
    if (stream.isClosed) return { status: 'exhausted', summary: 'Client disconnected' };
    trimImages(messages);
    ...
    if (toolUses.length === 0) {
      ...
      messages.push({
        role: 'user',
        content: [{ type: 'text', text: 'Use a tool (computer, navigate, or finish) to proceed.' }],
      });
      log.info('agent produced no tool use', { phase: opts.phase, text: text.slice(0, 120) });
      continue;
    }

    const results: Record<string, unknown>[] = [];
    for (const tu of toolUses) {
      const mapped = mapToolUse(tu.name, (tu.input ?? {}) as Record<string, unknown>);
      ...
      stream.send({ type: 'step', index: step, action: mapped.action.action, detail: result.detail });
