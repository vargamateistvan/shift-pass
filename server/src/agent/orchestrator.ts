 ...
 */
export async function runRotation(req: RotateRequest, stream: SseStream): Promise<void> {
  ...
  try {
    ...
    if (!config.anthropicApiKey) throw new Error('Server missing ANTHROPIC_API_KEY');
    ...
    stream.send({ type: 'phase', phase: 'navigating', message: `Opening ${url.host}…` });
    ...
    if (goalA.status === 'exhausted') throw new Error(`Could not request reset: ${goalA.summary}`);
    ...
    const email = await pollForResetEmail(req.googleAccessToken, url.host, startedAtMs, stream);
    ...
    stream.send({
      ...
      message: config.dryRun ? 'Filling new password (dry run)…' : 'Setting the new password…',
    });
    ...
    if (goalB.status === 'exhausted') throw new Error(`Could not set password: ${goalB.summary}`);

    // ---- Persist to the encrypted vault ----
    stream.send({ type: 'phase', phase: 'saving', message: 'Saving to encrypted vault…' });
    await saveEntry({
      id: randomUUID(),
      site: url.toString(),
      host: url.host,
